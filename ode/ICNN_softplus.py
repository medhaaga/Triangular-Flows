...
d = 2 + 2*k
xs = random.multivariate_normal(key, mean = jnp.array([ -0.125, -0.125, -3, -3]), cov = 0.01*jnp.identity(4), shape = (n,))
xs = jnp.exp(xs)
ys = jnp.zeros(shape = (n,2*k))

for i in range(n):
    if(i % 100 == 0): print(i)
    alpha, delta = xs[i,0], xs[i,3]
    ...
    #print(ys[i,])
key, subkey = random.split(key)
ys = ys + jnp.exp(random.multivariate_normal(subkey, mean = jnp.zeros(shape = (2*k,)), cov = 0.1*jnp.identity(2*k), shape = (n,)))
samps = jnp.concatenate([ys, xs[:,0].reshape((n,1)), xs[:,3].reshape((n,1))], axis=1)
...
plt.savefig('optim_curve_softplus.png')
...
plt.savefig('monotonicity_softplus.png')
plt.show()
...
plt.savefig('scatterplot_softplus.png')
plt.show

plt.rcParams["figure.figsize"] = (5,5)
for t in range(d):
    fig = sm.qqplot(z[:,t], line = '45')
    plt.savefig('qq_plots_softplus_'+str(t+1)+'.png')
    plt.show()


with open('params_softplus', 'wb') as file_handle:
